...
subroutine inner_bounce_trapz(h_i,h_j,f_i,f_j,theta_i,theta_j,y)
  ...
  y = SUM((-2*(Sqrt(f_j)*(2*h_i + h_j) + Sqrt(f_i)*(h_i + 2*h_j))*(theta_i - theta_j))/(3.*(f_i + f_j + 2*Sqrt(f_i*f_j))))
...
subroutine zero_cross_idx(y_arr,zero_idx,num_cross)
  ...
  real :: lam
  real(kind=8), dimension(size(b_arr))    :: roll_arr
...
subroutine bounce_wells(b_arr,theta_arr,lam,bounce_arr,bounce_idx,num_wells)
  ...
  real(kind=8), dimension(size(b_arr))                    :: zero_arr
  real(kind=8), dimension(:)                              :: zero_idx
  ...
  IF (MOD(num_cross,2).NEQ.0) THEN
    print 'Odd number of wells, brace for crash...'
  END IF
  ...
  IF (well_disc == 1 .AND. first_well_end == 1) THEN
    ...
    bounce_arr(1, 2) = (-(zero_arr(r_idx+1)*theta_arr(r_idx)) + zero_arr(r_idx)*theta_arr(r_idx+1))/(zero_arr(r_idx) - zero_arr(r_idx+1))
  ...
  ELSE IF (well_disc == 0 .AND. first_well_end == 1) THEN
    ...
    bounce_idx(1, 2) = r_idx
    ! linear interpolation to find theta_val
    bounce_arr(1, 1) = (-(zero_arr(l_idx+1)*theta_arr(l_idx)) + zero_arr(l_idx)*theta_arr(l_idx+1))/(zero_arr(l_idx) - zero_arr(l_idx+1))
    bounce_arr(1, 2) = (-(zero_arr(r_idx+1)*theta_arr(r_idx)) + zero_arr(r_idx)*theta_arr(r_idx+1))/(zero_arr(r_idx) - zero_arr(r_idx+1))
  END IF
  ...
  DO  do_idx = 1 + first_well_end, num_wells, 1
    ...
    ! linear interpolation to find theta_val
    bounce_arr(do_idx, 1) = (-(zero_arr(l_idx+1)*theta_arr(l_idx)) + zero_arr(l_idx)*theta_arr(l_idx+1))/(zero_arr(l_idx) - zero_arr(l_idx+1))
    bounce_arr(do_idx, 2) = (-(zero_arr(r_idx+1)*theta_arr(r_idx)) + zero_arr(r_idx)*theta_arr(r_idx+1))/(zero_arr(r_idx) - zero_arr(r_idx+1))
  END DO
...
subroutine bounce_average(h_arr,b_arr,theta_arr,lam,num_wells,bounce_idx,bounce_arr,bounce_ave)
  ...
  ! Allocate
  allocate ( bounce_ave(num_wells), l_idx(num_wells), r_idx(num_wells), l_cross(num_wells), r_cross(num_wells) )
  ...
  DO  do_idx = 1 + first_well_end, num_wells, 1
    ...
    ELSE IF
      h_l = h_arr(l_idx(do_idx)) + (l_cross(do_idx) - theta_arr(l_idx(do_idx)))/(theta_arr(l_idx(do_idx)+1) - theta_arr(l_idx(do_idx) )) * ( h_arr(l_idx(do_idx)+1) - h_arr(l_idx(do_idx)) )
      CALL left_bounce_trapz(h_l,h_arr(l_idx(do_idx)+1),0.0,f_arr(l_idx(do_idx)+1),l_cross(do_idx),theta_arr(l_idx(do_idx)+1),left)
    END IF
    ...
    ELSE IF
      h_r = h_arr(r_idx(do_idx)) + (r_cross(do_idx) - theta_arr(r_idx(do_idx)))/(theta_arr(r_idx(do_idx)+1) - theta_arr(r_idx(do_idx) )) * ( h_arr(r_idx(do_idx)+1) - h_arr(r_idx(do_idx)) )
      CALL right_bounce_trapz(h_arr(r_idx(do_idx)),h_r,f_arr(r_idx(do_idx)),0.0,r_cross(do_idx),theta_arr(r_idx(do_idx)),right)
    END IF
...
subroutine w_bounce(b_arr,dbdx_arr,dbdy_arr,sqrtg_arr,theta_arr,L_tot,lam,Delta_x,Delta_y,num_wells,w_psi_arr,w_alpha_arr,G_arr)
  ...
  ! Allocate
  allocate(w_psi_arr(num_wells),w_alpha_arr(num_wells),G_arr(num_wells),denom_arr(num_wells),numer_arr_alpha(num_wells),numer_arr_psi(num_wells))
...
subroutine AE_integrand(dlnTdx,dlnndx,Delta_x,z,w_alpha,w_psi,G,AE)
  ...
  AE = SUM((G*(w_psi*(-1 + Sqrt((-w_alpha + w_dia)**2 + w_psi**2)/Sqrt(w_alpha**2 + w_psi**2)) + w_alpha**2*(-1 + w_dia/w_alpha + Sqrt((-w_alpha + w_dia)**2 + w_psi**2)/Sqrt(w_alpha**2 + w_psi**2)))*z**2.5)/E**z)
end subroutine AE_integrand


! Calculate total AE
subroutine AE_total(dlnTdx,dlnndx,Delta_x,Delta_y,b_arr,dbdx_arr,dbdy_arr,sqrtg_arr,theta_arr,lam_res,z_res,z_min,z_max,AE_tot)
  implicit none
